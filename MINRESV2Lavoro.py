...
def test_givens_rotation():
    A=np.array([[0.8147,0.0975,0.1576],[0.9058,0.2785,0.9706],[0.1270,0.5469,0.9572],[0.9134,0.9575,0.4854],[0.6324,0.9649,0.80003]])
    Q,R= givens_rotation(A)
    print("Q",Q)
    print("R",R)
    print("Q.T@Q",Q.T@Q)
    print("Q.T@A",Q.T@A)
    print("R-1@A", A@np.linalg.inv(R))
...
def lanczos_minres2(A, b , x0=None,tol=1e-5, maxiter=None, func=__prodMV):
    ...
    b_norm:float=np.linalg.norm(w)
    beta1:float  = b_norm
    res:float=[] #residual array
    v0:np.ndarray = 0
    v1:np.ndarray = w/beta1
    alpha:float = 0
    beta2:float = 0
    Q = np.array(v1.reshape((len(v1),1)))
    Q = Q.reshape((len(v1),1))
    p1 = p0 = pk =0
    #Lanczos Iteraions
    res2:float=[]
    wk=v1.copy()
    for j in range(maxiter):
        ...
        #wj
        w-= v1 *  alpha #w - alpha_j q_j
        beta1 = beta2
        beta2 = np.linalg.norm(w) #  beta_j+1 = ||w_j||_2
        q_next= np.divide(w , beta2)
        v0=v1.copy()
        v1=q_next
        Q=np.hstack((Q,v1.reshape((len(v1),1))))
        ...
        G,R=givens_rotation(T)
        gamma=hypot(T[j,j],T[j+1,j])
        c=T[j,j]/gamma
        s=T[j+1,j]/gamma
        Q2=np.array([[c,s],[s,-c]])
        W=np.hstack([wk.reshape((len(wk),1)),v1.reshape((len(v1),1))])
        print(W.shape,Q2.shape)
        W=W@Q2
        wk=W[1]
        if j<4:
            print("Q",Q)
